 ...
 * @author Martin
 */
public class Or extends WorkSpaceObject {
     ...
     */
    public Or(Point position, int bits, int inputs) {
        super(position);
        this.bits = bits;
        this.inputs = Math.max(inputs, 2);
        super.setModel(new Model());
        this.OUT = Or.createModel(super.getModel(), this.inputs, this.bits, false);
    }
    ...
    @Override
    public Or cloneObject() {
        Or ret = new Or(Tools.copy(super.getPosition()), this.bits, this.inputs);
    ...
    public static IOPin createModel(Model model, int inputs, int bits, boolean outNegation) {
        ...
        //default model
        GOList.add(new Curve(-21, -21, 0, -10, 21, -21));
        GOList.add(new Curve(-21, -21, -21, 0, 0, 14));
        GOList.add(new Curve(21, -21, 21, 0, 0, 14));
        if (outNegation) {
        ...
        if (inputs > 3) {
            ...
            GOList.add(new Line(21, -21, width, -21));
            GOList.add(new Line(-21, -21, -width, -21));
        }
